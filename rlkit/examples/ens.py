...
from rlkit.torch.sac.ens import (
    EnsTrainer
)
...
class MLP(nn.Module):
    def __init__(self, in_dim, out_dim, hidden_size=256) -> None:
        ...
        self.mlp = nn.Sequential(
            ...
            nn.Linear(hidden_size, out_dim), nn.Softmax(out_dim)
        )
...
class ParallelTanhGaussianPolicy(nn.Module):
    ...
    def mixed_logprob(self, obs, action, weight): 
        logprobs = []
        for policy in self.policies:
            tanh_normal = policy(obs)
        ...
        mix_logprobs = logprobs * weight.sum(1)
        return mix_logprobs

    def mixed_action(self, obs, weight): 
        means = []
        for policy in self.policies:
            tanh_normal = policy(obs)
...
def experiment(variant, args):
    ...
    prior_weight = MLP(obs_dim, args.ensemble_size).to(ptu.device)
    ...
    prior_policy = ParallelTanhGaussianPolicy(policies.copy()).to(ptu.device)
