...
class EnsTrainer(TorchTrainer, LossFunction):
    def __init__(
        self,
        env,
        policy,
    ...
    def compute_loss(
        ...
        log_pi = log_pi.unsqueeze(-1).clamp(-100, 100)
    ...
    def mc_kl_divergence(self, obs, prior_weight, n_sample=5):
        policy_dist = self.policy(obs)

        prior_dist = []
        for prior in self.prior_policy:
            prior_dist.append(prior.dist(obs))

        samples = [policy_dist.sample() for _ in range(n_sample)]
        kl_div = []
        for x in samples:
            policy_log_prob = policy_dist.log_prob(x).clamp(-100, 100)
    ...
    def compute_prior_loss(self, obs):
        ...
        q_mixes = []
        for prior in self.prior_policy:
            prior_action, _ = prior.dist(obs).rsample_and_logprob()
            q_prior = torch.min(
            self.qf1(
                obs.unsqueeze(0).repeat(self.ensemble_size, 1, 1),
